...
router.get('/:id', function (req, res) {
	controller.conn.getConnection(function(err, connection){
		...
		connection.query('select game_date, home_team_id, away_team_id, t.team_name as hometeam,' +
		' t2.team_name as awayteam from games left join teams t on games.home_team_id = t.id' +
		' and games.league_id = ' + req.params.id +
		' left join teams t2 on games.away_team_id = t2.id order by game_date',
		function (err, games) {
	        if (err) {
	        	console.log(err);
	        	connection.release();
