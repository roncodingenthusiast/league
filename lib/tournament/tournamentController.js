...
router.get('/:id', function(req, res){
	controller.conn.getConnection(function(err, connection){
		...
		connection.query('select * from tournaments where id='+req.params.id, function (err, tournaments) {
	        if (err) {
	        	console.log(err);
	        	connection.release();
	        	return res.status(500).send("There was a problem finding the tournaments.");
...
router.delete('/:id', function(req, res){
	controller.conn.getConnection(function(err, connection){
		...
		connection.query('delete from tournaments where id='+req.params.id, function (err, tournaments) {
	        if (err) {
	        	console.log(err);
	        	connection.release();
	        	return res.status(500).send("There was a problem finding the tournaments.");
...
router.put('/:id', function(req, res){
	controller.conn.getConnection(function(err, connection){
		...
		connection.query('update tournaments set ? where id='+req.params.id, postData, function (err, tournaments) {
	        if (err) {
	        	console.log(err);
	        	connection.release();
	        	return res.status(500).send("There was a problem finding the tournaments.");
