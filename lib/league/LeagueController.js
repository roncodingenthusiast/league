...
router.get('/:id', function(req, res){
	controller.conn.getConnection(function(err, connection){
		...
		function(errorQuerying, leagueQueried){
			...
			console.log('leagueQueried', leagueQueried);
...
router.put('/:id', function(req, res){
	controller.conn.getConnection(function(err, connection){
		...
		var currentBody  = _.get(req, 'body.current');
		var updateBody  = _.get(req, 'body.update');
		console.log('body current', currentBody);
		console.log('body update', updateBody);
		var postData = {
			id: req.params.id,
			league_name: 'update'
		};
		connection.query('update league set ? where id='+postData.id, req.body,
